    ...
    var body: some View {
        VStack {
            ...
            if user.currentMode == 0 && user.chunithm.rating.best.count > 0 {
                RatingShortLook(user: user, length: min(50, user.chunithm.rating.best.count + user.chunithm.rating.new.count), coverUrl: ChunithmDataGrabber.getSongCoverUrl(source: 0, musicId: String(user.chunithm.rating.best.first?.musicId ?? 0)), gradient: user.homeUseCurrentVersionTheme ? [nameplateThemedChuniColors.first!, nameplateThemedChuniColors.last!] : [nameplateDefaultChuniColorBottom, nameplateDefaultChuniColorTop])
